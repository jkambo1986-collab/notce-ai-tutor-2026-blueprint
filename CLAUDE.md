...
- `services/api.ts` is the single source of all backend calls. JWT access token lives in `localStorage` as `auth_token` (refresh as `refresh_token`) and is attached as `Bearer` per-request. There is **no global fetch wrapper / interceptor** — each method re-reads the token and sets headers itself.
