...
const MockStudySession: React.FC<MockStudySessionProps> = ({ sessionId, initialData, onExit }) => {
    ...
    // --- RENDER: EMPTY/MALFORMED QUESTION GUARD ---
    // A question with no stem or no options can occur for a resumed/stale session
    // whose stored data is empty (e.g. an old session created before bank-only
    // serving). Without this guard the card renders blank and `options.map` below
    // would throw. Offer a graceful recovery instead.
    if (!currentQuestion?.stem || !(currentQuestion?.options?.length)) {
        return (
            <div className="h-full bg-canvas flex flex-col items-center justify-center p-8 text-center">
                <div className="w-14 h-14 rounded-2xl bg-amber-100 text-amber-600 flex items-center justify-center mb-4">
                    <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01M5 19h14a2 2 0 001.84-2.75L13.74 4a2 2 0 00-3.48 0L3.16 16.25A2 2 0 005 19z" /></svg>
                </div>
                <h3 className="text-xl font-bold text-ink mb-2">This question didn't load</h3>
                <p className="text-slate-500 mb-6 max-w-sm">The session data looks incomplete. Skip to the next question, or head back and start a fresh session.</p>
                <div className="flex gap-3">
                    <Button onClick={handleNextQuestion} loading={isLoading}>Next question</Button>
